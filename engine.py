...
FAILURE_RATE = float(os.getenv("FAILURE_RATE", "0.1"))  # 10% de fallos
...
class ResponseEngine:
    ...
    def compute(self, query: QueryRequest) -> dict:
        # Simular fallos si está habilitado
        if SIMULATE_FAILURES and random.random() < FAILURE_RATE:
            raise Exception("Simulated temporary failure in Response Engine")
