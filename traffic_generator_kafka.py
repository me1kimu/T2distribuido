...
def run(requests_n: int, distribution: str, sleep_ms: int, seed: int) -> None:
    ...
    try:
        ...
        logger.error(f"No se pudo conectar a Kafka en {KAFKA_BROKER}: {e}")
        return
    
    logger.info(f"Publicando {requests_n} consultas en tópico '{KAFKA_TOPIC}'")
    ...
    logger.info(f"Consultas publicadas: {sent}")
    logger.info(f"Errores: {errors}")
    logger.info(f"Tiempo total: {elapsed:.2f}s")
    logger.info(f"Throughput: {sent / elapsed:.2f} msg/s")
